...
[source, cypher]
MATCH (m:Movies) where m.released > 2010 and m.released < 2015 RETURN m
...
ON MATCH SET p.createdAt = timestamp()
ON CREATE SET released = "2020" and p.lastLoggedInAt = timestamp()
Return m
...
[source, cypher]
MATCH (p:Person), (m:Movie)
WHERE p.name = "<Your Name>" and m.name = "Cloud Atlas"
CREATE (p)-[w:WATCHED]->(m)
RETURN type(w)
...
[source, cypher]
MATCH (m:Movie {title: 'Cloud Atlas'})<-[d:DIRECTED]-(p:Persom) return p.name
...
[source, cypher]
MATCH (p:Person)-[relatedTo]-(m:Movie {title: "Cloud Atlas"}) return people.name, type(relatedTo)
